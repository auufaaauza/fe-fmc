...
function AdminProgramDetailContent() {
  ...
  useEffect(() => {
    async function loadProgram() {
      try {
        const response = await api.get(`/study-programs/${params.id}`);
  ...
  if (!program) {
    return (
      <div className="nb-card p-12 text-center">
        <p className="font-medium text-slate-700">Data program studi tidak ditemukan.</p>
        <Button asChild variant="plain" className="mt-4">
          <Link href={fromUrl || "/admin/program-studi"}>
  ...
  const backHref = fromUrl || "/admin/program-studi";
  const backLabel = fromUrl?.includes("/admin/siswa")
